...
이 서비스는 공개 지식 질문과 승인책임자(`X-Approver-Token`) 전용의 세 가지 흐름을 제공한다. SQL
생성·SQL 검증·후보 탐색은 흐름별 전용 엔드포인트 없이 **기존 `POST /query` 자체**가 처리한다
(별도 엔드포인트 `/assist`를 만들었다가 다시 합쳤다) — 별도 분류기 없이 기존 Multi-Agent
Supervisor가 입력을 보고 스스로 담당 에이전트에 위임한다(`src/pipeline.py`의 `run_via_supervisor`
참고). `X-Approver-Token` 헤더 자체가 없으면 SQL 원문이든 자연어든 Supervisor를 거치지 않고
기존 계약 그대로 `run_query`로 처리한다 — 헤더가 있을 때만(값이 맞든 틀리든) Supervisor 전체
라우팅이 열리고, 보호 Agent가 전용 Tool을 실행할 때만 토큰 값을 검사한다. 후보 선택 후 진단만은 전용 엔드포인트
`POST /candidates/diagnose`로 별도로 둔다 — 사용자가 목록에서 카드를 클릭해 나온 선택이라
분류할 자연어가 없는 결정적 액션이기 때문이다.

| 흐름 | 설명 |
|---|---|
| **SQL 생성** | 비즈니스 요구사항을 입력하면 `query_planner_agent`가 처리 계획을 먼저 제시한다. 사람이 승인한 뒤에만 Plan-Execute 패턴으로 스키마를 확인하고 SELECT 초안을 생성한다 |
...
- `POST /candidates/diagnose`는 몸체와 무관하게 항상 `X-Approver-Token`을 검사한다(누락 401,
  불일치 403) — 자연어가 없는 결정적 선택이라 조건부 검사가 필요 없다.
- `POST /query`는 `question`만 받는다. 토큰 헤더 자체가 없으면 legacy `run_query`로 처리하고
  (SQL 원문 포함, 항상 200), 헤더가 있으면 Supervisor가 Agent를 고른다. `query_planner_agent`,
  `sql_validator_agent`, `candidate_search_agent`는 Tool 실행 직전에 토큰 값을 검사해 불일치는
  403이며, 지식·일반 요청(`explain_agent`/`knowledge_agent`/`general_agent`)은 토큰 없이 동작한다.
  UPDATE·DELETE·DROP 등 변경/DDL SQL은 헤더 유무와 무관하게 항상 코드로 결정적으로 차단한다.
- 승인된 요청: 사용자 식별자, 시각, 작업, 결과를 감사 기록으로 남긴다. 원문 토큰은 저장하지 않는다.
- 권한 검증 실패: SQLcl MCP 호출과 SQLite 쓰기를 전혀 수행하지 않는다. Supervisor는 라우팅만 하고,
  보호된 전용 Tool의 인증 경계가 DB·저장소 접근보다 먼저 동작한다.
...
기존 제출 계약인 `POST /query {"question": str}`는 유지하며, 세 흐름과 후보 진단도 같은
엔드포인트로 처리한다. 모든 요청 유형은 동일한 응답 계약
`{"status", "answer", "contexts", "trace"}`를 따른다.

| 요청 유형 | 경로 | 요청 본문 | 토큰 |
|---|---|---|---|
| 진단 질문 / SELECT 직접 입력 (헤더 없음) | `POST /query` | `{"question": str}` | 불필요 |
| SQL 생성·SQL 검증·후보 탐색(Supervisor가 내부 판단) | `POST /query` | `{"question": str}` | 선택된 보호 Agent에서 필수 |
| 후보 선택 후 진단 | `POST /candidates/diagnose` | `{"sql_id": str}` | 필수(항상 검사) |
...
- **도구**: SQLcl MCP 도구(스키마 조회, 운영 SQL 후보 탐색, `EXPLAIN PLAN`/`DBMS_XPLAN` 실행계획 조회,
  ...
  개선안은 advisory 전용이며, `gather_stats`·`create_index` 같은 위험 쓰기 도구는 공개 API에 노출하지 않는다
- **서브 에이전트**: Supervisor가 explain_agent(실행계획 분석)·knowledge_agent(지식베이스 검색)·
  query_planner_agent(SQL 생성 계획)·sql_validator_agent(SQL 검증)·candidate_search_agent(운영 후보 탐색)에
  위임한다. 구조화된 최종 응답은 각 파이프라인의 finalize 단계가 만든다
- **데이터**: Docker(`docker-compose.yml`, gvenzl/oracle-free)로 띄우는 실제 Oracle Database Free 인스턴스에
  ...
  Oracle 튜닝 이론 지식베이스 문서 5종(`data/knowledge/*.md`, 풀스캔/카디널리티/비-sargable/조인/통계 원인·개선 패턴)
...
- 개선안은 조언(advisory)만 제공하며 인덱스 생성이나 통계 재수집을 자동으로 실행하지 않는다. 위험 쓰기 작업용 공개 API는 제공하지 않는다
